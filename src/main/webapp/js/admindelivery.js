     ...
     var PAGESIZE = 15;
        ...
        function buildTable(goodsName,pageNumber,pageSize) {
        	 var url =  urlRootContext + "/delivery/list"; //请求的网址
         ...
         if (dataList.length > 0 ) {
             $(dataList).each(function(){//重新生成
            	...
            	+'<td>' + this.orderId + '</td>'
            	+'<td>' + this.price + '</td>'
            	+'<td>' + toDate(this.time) + '</td>'
            	+'<td>' + this.orderAddress + '</td>'
            	+'<td style="width: 150px;"> <div class="btn-group"> <a href="" class="btn btn-default" id="goodsModify" onclick="Modify('+this.id+');return false;">详情</a>'
            	+' </div> </td>'
            	+'</tr>'
            	 );
